...
def cluster(G, eps=0.2, c=0.1, verbose=0, check_laminar=False):
    ...
    Sample = []
    for v in G.vertices:
        operation_counter.add(1, tag="build_verticies")
    ...
    def sparse_tester(v):
        operation_counter.add(1, tag="sparse_tester")
        low_epsP = low(v, epsP)
        operation_counter.add(1 * len(low_epsP), tag="sparse_tester")
    ...
    def C_filter(u, v, low_epsP):
        # u is in low_epsP = low(v, epsP)
        operation_counter.add(1, tag="C_filter")
    ...
    def C_tilde(v, low_epsP):
        operation_counter.add(1 + len(low_epsP), tag="C_tilde")
    ...
    operation_counter.add(1 * len(Sample))
    ...
    for v in D:
        ...
        for u in Cv:
            operation_counter.add(1, tag="Double nested")
    ...
    while len(candidates) > 0:
        ...
        operation_counter.add(1, tag="remove_candidates")
    ...
    for v, l in labels.items():
        operation_counter.add(1, tag="make_labels")
        if l in clusters:
            clusters[l] |= {v}
        else:
            clusters[l] = {v}
